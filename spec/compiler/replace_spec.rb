...
describe DabNode do
  it 'allows to replace child' do
    ...
    expect(body.all_nodes(DabNodeSymbol).map(&:symbol)).to eq(%i[p1 p3])
  end
  ...
  it 'allows to replace child with array' do
    ...
    expect(body.all_nodes(DabNodeSymbol).map(&:symbol)).to eq(%i[p1 p3 p4 p5])
  end
  ...
  it 'allows to replace child with array with the child itself' do
    ...
    expect(body.all_nodes(DabNodeSymbol).map(&:symbol)).to eq(%i[p1 p2 p3 p4])
  end
end
