...
int des_screen_width() { return 256; }
int des_screen_height() { return 224; }
...
{    
  ...
  //   fprintf(stderr,"loaded %d x %d\n", image.width(), image.height());
    ...
    fprintf(stderr,"[desx] Loaded '%s': %dx%d pixels, %d color type (PNG_COLOR_TYPE_PALETTE = %d), %d bit depth\n",
        path, (int)width, (int)height, color_type, (int)PNG_COLOR_TYPE_PALETTE, bit_depth);

    if (color_type != PNG_COLOR_TYPE_PALETTE) {
        fprintf(stderr, "[desx] must be indexed png\n");
        exit(1);
    }
    ...
    if (png_get_PLTE(png, info, &palette, &num_palette) != PNG_INFO_PLTE) {
        fprintf(stderr, "[desx] no palette\n");
        exit(1);
    }

    fprintf(stderr, "Palette has %d colors.\n", num_palette);
    if (num_palette != 16) {
        fprintf(stderr, "[desx] palette must have 16 colors\n");
        exit(1);
    }
