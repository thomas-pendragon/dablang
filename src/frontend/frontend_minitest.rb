...
class MinitestSpec
  ...
  def read_test_file
    ...
    # body = base[:expect_ok]
    # run = 'minitest0'
    compile_error = nil # base[:expect_compile_error].presence
    runtime_error = nil # base[:expect_runtime_error].presence
    # if compile_error
...
if $autorun
  read_args!
  raise 'no dab' unless $settings[:input].downcase.end_with?('.dab')

  test = MinitestSpec.new
